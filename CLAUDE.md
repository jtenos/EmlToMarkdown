...
- **[convert.go](convert.go)** — `Convert` (file path) wraps `convert` (arbitrary `io.Reader` + injected `imageFetcher`). Builds the fixed header block (`### EMAIL:`, Date normalized to UTC, From/To normalized to `Name <addr>`, optional `Attachments (skipped)` list). Handles RFC 2047 encoded-word decoding.
- **[mime.go](mime.go)** — `walkPart` recursively flattens the MIME tree into `mailParts` (first `text/html`, first `text/plain`, inline images keyed by Content-ID, and attachment names in document order). `decodeBody` reverses Content-Transfer-Encoding (base64 with whitespace stripped, quoted-printable). Note: `mime/multipart` transparently decodes quoted-printable parts, so those pass through already decoded.
- **[body.go](body.go)** — `buildBody` prefers the HTML part (falling back to plain text). `htmlToMarkdown` rewrites every `<img>` to be self-contained before conversion: `cid:` refs resolved from inline images, `http(s)` fetched and inlined, all as base64 data URIs. Unresolvable images become a `| Failed to load image |` GFM table via a placeholder-token swap done *after* Markdown conversion (so the table markup survives HTML escaping). HTML→Markdown uses `JohannesKaufmann/html-to-markdown/v2` with base + commonmark + table + strikethrough plugins.
...
Test fixtures live in [testdata/](testdata/): `plain.eml`, `html_inline.eml`, `html_remote.eml`.
